data() {
  return {
    ...
    insertedProject: {
      ...
      startDate: '',
      endDate: '',
      // Otros campos necesarios para el proyecto
    },
    errors: {
      name: [],
      description: [],
      startDate: [],
      endDate: [],
      // Otros errores para los campos del proyecto
    },
    rules: {
      name: {
        required: true,
      ...
      // Otros campos y reglas de validación necesarios para el proyecto
    }
  }
},
...
methods: {
  validateField(field) {
    this.errors[field] = [];
    const value = this.insertedProject[field];
    if (!value && this.rules[field].required) {
      this.errors[field].push('Este campo es obligatorio');
    }

    if (field === 'endDate' && value) {
      const startDate = new Date(this.insertedProject.startDate);
      const endDate = new Date(value);
      if (endDate < startDate) {
        this.errors[field].push('La fecha de finalización debe ser posterior a la fecha de inicio');
      }
    }
  },

  validateAllProjectFields() {
